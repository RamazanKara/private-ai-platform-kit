...
This document draws an explicit boundary around what Private AI Platform Kit (v0.12.0) is and is not,
...
tooling, and operational runbooks" (see [README Support Boundaries](../README.md) and
[ROADMAP Remaining External / Operator-Owned Work](../ROADMAP.md)). Everything below is read against
that boundary.
...
- **It does not provision cloud infrastructure.** No Terraform/CloudFormation for VPCs, subnets, node
  ...
  ([README Customer-Owned Kubernetes](../README.md)).
...
- **Identity provider.** The gateway validates API-key hashes and optional JWT/JWKS, but the kit does
  ...
  Hardening; [runbooks/oidc-jwks-rotation.md](../runbooks/oidc-jwks-rotation.md)).
...
- **No multi-node distributed serving operator.** Multi-node serving needs the LeaderWorkerSet (or
  ...
  pipeline-parallel flags ([runbooks/gpu-capacity.md](../runbooks/gpu-capacity.md)); installing and
...
| --- | --- | --- |
| **Operational Excellence** | GitOps with Argo CD auto-sync (`deploy/gitops/argocd`, `deploy/clusters/{local,customer}`); ~28 runbooks (`runbooks/`); release gates that require current evidence (`platform/slo/release-gates.yaml`, `make release-gate-strict`); evidence packs (`make evidence`); chaos/upgrade drills ([runbooks/chaos-drills.md](../runbooks/chaos-drills.md), [runbooks/upgrade.md](../runbooks/upgrade.md)); API/config contract snapshots (`make api-contract`, `make config-contract`); tamper-evident SHA-256 audit chain in the gateway (`_chain_audit_event` in `src/inference-gateway/app/main.py`). | Owning the incident process, on-call, and change management; keeping evidence current per release. |
...
- [Roadmap](../ROADMAP.md) — the operator-owned work that remains external to the kit.
