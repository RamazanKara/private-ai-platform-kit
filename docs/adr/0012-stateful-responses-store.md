...
- **Storage:** a `ResponseStore` protocol with a `MemoryResponseStore` (local/tests) and a
  ...
  tenant-scoped — a tenant can neither read nor delete another tenant's responses.
...
- The gateway can persist raw conversation content when an operator opts in — a deliberate
  departure from the redacted-only default, bounded by: off-by-default, per-tenant isolation, a
