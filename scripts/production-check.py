...
def require_lock_contains_pins(errors: list[str], requirements: Path, lockfile: Path, expected: dict[str, str]) -> None:
    ...
    for name, version in expected.items():
        require(
            ...
            f"{lockfile.relative_to(ROOT)} must include pinned dependency {name}=={version} from {requirements.relative_to(ROOT)}",
        )
...
def check_values_and_docs(errors: list[str]) -> None:
    ...
    require(
        errors,
        production_doc.startswith("# Production Readiness Matrix"),
        "production readiness document must keep its title",
    )
    require(
        errors, "## Required Controls" in production_doc, "production readiness document must list required controls"
    )
    require(
        errors,
        "| Area |" in production_doc and "| Validation |" in production_doc,
        "production readiness controls must remain tabular",
    )
    policies = (ROOT / "deploy/policies/kyverno/policies.yaml").read_text()
