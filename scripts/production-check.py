...
def check_values_and_docs(errors: list[str]) -> None:
    ...
    require(errors, os.access(ROOT / "scripts/agent-sandbox-install.sh", os.X_OK), "scripts/agent-sandbox-install.sh must be executable")
    require(errors, os.access(ROOT / "scripts/agent-sandbox-smoke.sh", os.X_OK), "scripts/agent-sandbox-smoke.sh must be executable")
